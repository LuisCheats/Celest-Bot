...
async function ensureTmpDir() {
    try {
        ...
        console.log('Directorio tmp existe:', tmp)
        return true
    } catch {
        console.log('Directorio tmp no existe, creando:', tmp)
        try {
            // Intentar con promises.mkdir primero
            await fs.promises.mkdir(tmp, { recursive: true, mode: 0o755 })
            ...
            try {
                ...
                console.error('Ruta completa:', tmp)
...
function sticker2(img, url) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            await ensureTmpDir()
            inp = path.join(tmp, +new Date + '.jpeg')
...
function sticker6(img, url) {
    return new Promise(async (resolve, reject) => {
        ...
        try {
            ...
            await ensureTmpDir()
            tmpFile = path.join(tmp, `${+ new Date()}.${type.ext}`)
